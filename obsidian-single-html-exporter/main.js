...
${titleHtml}${fullHtml}
    ...
    let layoutOverrideCss = `
		...
		.markdown-rendered .mermaid-link {
			display: block;
			cursor: zoom-in;
    ...
    if (enableImageZoom) {
      ...
      fullHtml = this.wrapMermaidInLinks(fullHtml);
    }
...
${html}`;
  ...
  wrapMermaidInLinks(html) {
    const mermaidRegex = /<div[^>]*class=["'][^"']*mermaid[^"']*["'][^>]*>([\s\S]*?<svg[\s\S]*?<\/svg>[\s\S]*?)<\/div>/gi;
    return html.replace(mermaidRegex, (match, content) => {
      ...
      const inlineStyle = "font-family: ui-sans-serif, system-ui, sans-serif; color: black; background-color: white;";
      if (svg.includes('style="')) {
        svg = svg.replace('style="', `style="${inlineStyle} `);
      } else {
        svg = svg.replace("<svg", `<svg style="${inlineStyle}"`);
      }
      const base64 = Buffer.from(svg).toString("base64");
      const dataUri = `data:image/svg+xml;base64,${base64}`;
      return `<a href="${dataUri}" target="_blank" class="image-link mermaid-link">${match}</a>`;
    });
  }
};
